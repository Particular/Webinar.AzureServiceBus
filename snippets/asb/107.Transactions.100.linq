...
static async Task MainAsync()
{
	...
	await EnsureQueueExists("myAnotherQueue", credentials, serviceBusUri).ConfigureAwait(false);
	...
	const int numberOfMessagesInTransaction = 101;
	var messages = Enumerable.Range(1, numberOfMessagesInTransaction).Select(index => new BrokeredMessage($"message # {index}")).ToList();
		
	var sendTasks = new List<Task>();
	using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
	{
		"Preparing messages to be send".Dump();
		foreach (var message in messages)
		{
			...
			sendTasks.Add(sender.SendAsync(message));
		}

		$"Sending {numberOfMessagesInTransaction} messages to myQueue".Dump();
