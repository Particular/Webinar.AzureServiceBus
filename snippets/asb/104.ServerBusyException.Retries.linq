...
static async Task MainAsync()
{
	...
	while(x != "x")
	{
		...
		var message = new BrokeredMessage("some data");
		await sender.SendWithRetriesAsync(s => s.SendAsync(message), s => s.SendAsync(message.Clone()), TimeSpan.FromSeconds(10), 5).ConfigureAwait(false);
...
{
	public static async Task SendWithRetriesAsync(this MessageSender sender, 
												  Func<MessageSender, Task> action, 
												  Func<MessageSender, Task> retryAction, 
												  TimeSpan wait, 
												  int maxRetryAttempts, 
												  int usedRetryAttempts = 0)
	{
		try
		{	        
