...
// uncomment to fill up the queue
...
//	stopwatch.Stop();
//	$"Sending messages took {stopwatch.ElapsedMilliseconds} millis:".Dump();


	var factoryPerReceiver = true;
	var numberOfReceivers = 16;
	var receivers = new List<MessageReceiver>();
	for(var i= 0; i < numberOfReceivers; i++){
		var receiver = await CreateReceiver("myQueue", credentials, serviceBusUri, ReceiveMode.PeekLock, factoryPerReceiver).ConfigureAwait(false);
    ...
    timer.Elapsed += ( sender, e ) => {
		var count = Interlocked.Exchange(ref received, 0);
		$"Received {count} messages this second:".Dump();
	};
    timer.Start();	  	
	
	foreach(var receiver in receivers){
		receiver.OnMessage(m => Interlocked.Increment(ref received)	,  
				new OnMessageOptions
	            {
	                AutoComplete = true,
	                MaxConcurrentCalls = 1 //increase if slower call
	            });
	}	
    
	Util.ReadLine("Press any key to stop receivers");
	
	$"Stopping".Dump();
	timer.Stop();
	await Task.WhenAll(receivers.Select(x => x.CloseAsync())).ConfigureAwait(false);
}
...
static async Task<MessageReceiver> CreateReceiver(string queuename, TokenProvider credentials, Uri serviceBusUri, ReceiveMode receiveMode, bool newFactory){
	...
	if(!newFactory){
		...
		receiver = await factory.CreateMessageReceiverAsync(queuename, receiveMode).ConfigureAwait(false);
	}
	else{
		var factorySettings = new MessagingFactorySettings{
			TokenProvider = credentials,
		...
		receiver = await f.CreateMessageReceiverAsync(queuename, receiveMode).ConfigureAwait(false);
	}
	receiver.PrefetchCount = 256;
	return receiver;
}
